package cn.chenyilei.center.spi.utils;


import cn.chenyilei.center.spi.convert.Convert;
...
public class MyReflectUtils {
    ...
    public static <T> Constructor<T> getConstructor(Class<T> clazz, Class<?>... parameterTypes) {
        ...
        for (Constructor<?> constructor : constructors) {
            ...
            if (MyClassUtils.isAllAssignableFrom(pts, parameterTypes)) {
                // 构造可访问
    ...
    public static void setFieldValue(Object obj, Field field, Object value) {
        ...
        if (null != value) {
            ...
            value = MyClassUtils.getDefaultValue(fieldType);
        }
    ...
    public static Set<String> getPublicMethodNames(Class<?> clazz) {
        ...
        if (MyArrayUtils.isNotEmpty(methodArray)) {
    ...
    public static Method getMethodOfObj(Object obj, String methodName, Object... args) throws SecurityException {
        if (null == obj || MyStrUtils.isBlank(methodName)) {
            return null;
        }
        return getMethod(obj.getClass(), methodName, MyClassUtils.getClasses(args));
    }
    ...
    public static Method getMethod(Class<?> clazz, boolean ignoreCase, String methodName, Class<?>... paramTypes) throws SecurityException {
        if (null == clazz || MyStrUtils.isBlank(methodName)) {
            return null;
        }

        final Method[] methods = getMethods(clazz);
        if (MyArrayUtils.isNotEmpty(methods)) {
            for (Method method : methods) {
                if (MyStrUtils.equals(methodName, method.getName(), ignoreCase)) {
                    if (MyClassUtils.isAllAssignableFrom(method.getParameterTypes(), paramTypes)) {
                        return method;
                    }
                }
            }
    ...
    public static Method[] getMethods(Class<?> clazz, Predicate<Method> filter) throws SecurityException {
        ...
        return MyArrayUtils.filter(getMethods(clazz), filter);
    }
    ...
    public static Method[] getMethodsDirectly(Class<?> beanClass, boolean withSuperClassMethods) throws SecurityException {
        ...
        while (searchType != null) {
            ...
            if (null == allMethods) {
                ...
                allMethods = MyArrayUtils.append(allMethods, declaredMethods);
            }
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T> T newInstance(String clazz) throws IllegalArgumentException {
        try {
            return (T) Class.forName(clazz).newInstance();
        } catch (Throwable e) {
            throw new IllegalArgumentException(MyStrUtils.format("Instance class [{}] error!", clazz), e);
        }
    }
     ...
     * @param params 构造函数参数
     * @return 对象
     * @throws IllegalArgumentException 包装各类异常
     */
    public static <T> T newInstance(Class<T> clazz, Object... params) throws IllegalArgumentException {
        if (MyArrayUtils.isEmpty(params)) {
        ...
        final Class<?>[] paramTypes = MyClassUtils.getClasses(params);
        final Constructor<T> constructor = getConstructor(clazz, paramTypes);
        if (null == constructor) {
            throw new IllegalArgumentException("No Constructor matched for parameter types: " + Arrays.stream(params).map(Object::getClass).collect(Collectors.toList()));
        }
        try {
    ...
    public static <T> T newInstanceIfPossible(Class<T> beanClass) {
        ...
        for (Constructor<T> constructor : constructors) {
            ...
            setAccessible(constructor);
            try {
                return constructor.newInstance(MyClassUtils.getDefaultValues(parameterTypes));
