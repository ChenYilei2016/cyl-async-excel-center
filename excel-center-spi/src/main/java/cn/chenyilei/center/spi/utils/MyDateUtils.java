...
/**
 * 时间工具类MyCharUtils@author xuechao.sxc
 */
public class MyDateUtils {
    ...
    public static Date parse(CharSequence dateCharSequence) {
        if (MyStrUtils.isBlank(dateCharSequence)) {
            return null;
        }
        String dateStr = dateCharSequence.toString();
        // 去掉两边空格并去掉中文日期中的“日”和“秒”，以规范长度
        dateStr = MyStrUtils.removeAll(dateStr.trim(), '日', '秒');
        ...
        // 没有更多匹配的时间格式
        throw new IllegalArgumentException("No format fit for date String:" + dateStr);
    }
    ...
    private static Date parse(CharSequence dateStr, DateFormat dateFormat) {
        if (MyStrUtils.isBlank(dateStr)) {
            throw new IllegalArgumentException("dataStr is blank");
        }
        try {
            ...
            throw new IllegalArgumentException(MyStrUtils.format("Parse [{}] with format [{}] error!", dateStr, pattern), e);
        }
    }
