...
/**
 * 数字工具类<br>MyCharUtils@author xuechao.sxc
 */
public class MyNumberUtils {
    ...
    public static Number parseNumber(String numberStr) {
        ...
        try {
            ...
            throw new IllegalArgumentException("parseNumber exception for:" + numberStr, e);
        }
    }
    ...
    public static Number parseNumber(String text, Class targetClass) {
        ...
        throw new IllegalArgumentException("invalid number class:" + targetClass);
    }
    ...
    public static BigDecimal createBigDecimal(String str) {
        if (str == null) {
            return null;
        } else if (MyStrUtils.isBlank(str)) {
    ...
    private static String removeNumberFlag(String number) {
        ...
        if ('D' == lastCharUpper || 'L' == lastCharUpper || 'F' == lastCharUpper) {
            number = MyStrUtils.subPre(number, lastPos);
        }
        return number;
    }
    ...
    public static int parseInt(String number) throws NumberFormatException {
        if (MyStrUtils.isBlank(number)) {
            return 0;
        }

        // 对于带小数转换为整数采取去掉小数的策略
        number = MyStrUtils.subBefore(number, MyCharUtils.DOT, false);
        if (MyStrUtils.isEmpty(number)) {
            return 0;
        }

        if (MyStrUtils.startWithIgnoreCase(number, "0x")) {
            // 0x04表示16进制数
