package cn.chenyilei.center.spi.convert;


import cn.chenyilei.center.spi.utils.MyArrayUtils;
import cn.chenyilei.center.spi.utils.MyCharUtils;
...
public abstract class AbstractConverter<T> implements Converter<T>, Serializable {
    ...
    public T convert(Object value, T defaultValue) {
        ...
        if (null == targetType && null == defaultValue) {
            throw new NullPointerException(MyStrUtils.format("[type] and [defaultValue] are both null for Converter [{}], we can not know what type to convert !", this.getClass().getName()));
        }
        if (null == targetType) {
            // 目标类型不确定时使用默认值的类型
        ...
        if (null == defaultValue || targetType.isInstance(defaultValue) || MyClassUtils.isBasicType(targetType)) {
            ...
            throw new IllegalArgumentException(MyStrUtils.format("Default value [{}] is not the instance of [{}]", defaultValue, targetType));
        }
    }
    ...
    protected String convertToStr(Object value) {
        ...
        } else if (MyArrayUtils.isArray(value)) {
            return MyArrayUtils.toString(value);
        } else if (MyCharUtils.isChar(value)) {
            //对于ASCII字符使用缓存加速转换，减少空间创建
            return MyCharUtils.toString((char) value);
        }
        return value.toString();
    }
    ...
    public Class<T> getTargetType() {
        return (Class<T>) MyClassUtils.getTypeArgument(getClass());
    }
}
